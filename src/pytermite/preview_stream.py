...
class PreviewStream:
    def __init__(self, serials, stop_event, logger):
        ...
        self.ips = {
            f"172.2{serial_nr[-3]}.1{serial_nr[-2:]}.51:8080": self.ports[idx]
            for idx, serial_nr in enumerate(self.serials)
        }
        self.root = tk.Tk()
    ...
    async def show_streams(self):
        ...
        for ip, port in self.ips.items():
            ...
            threading.Thread(
                target=UDPReceiver,
                args=(self.canvas, row, col, ip, port, canvas_size),
                daemon=True,
            ).start()
...
class UDPReceiver:
    ...
    def _run(self):
        ...
        while raw_bytes := process.stdout.read(frame_size):
            ...
            canvas.after(
                0,
                lambda: canvas.create_image(
                    self.column * width, self.row * height, image=imageTk
                ),
            )
